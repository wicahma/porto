  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    animationFrame.current = requestAnimationFrame(() => {
      ...
      itemsNodeList.forEach((item) => {
        ...
        const scale = 1 + Math.max(0, 0.6 - (distance + 80) / maxDistance);
        const opacity = Math.max(0.05, 1 - distance / (maxDistance * 1.2));
        const blur = Math.min(2, (distance / maxDistance) * 2);

        const el = item as HTMLElement;
        el.style.transition =
          "transform 0.15s ease-out, filter 0.15s ease-out, opacity 0.15s ease-out";
        el.style.transform = `scale(${scale}) translate(${scale + 40}px)`;
  ...
  return (
    ...
    >
      {debug && (
        <div className="flex justify-end bg-neutral-600 gap-1 rounded px-1 py-[2px] text-[10px] items-center absolute top-5 right-5 z-50 pointer-events-none">
          ...
          >
            Stay:{" "}
            <strong>
              {itemIndex === hoverInd && hoverInd !== null ? "true" : "false"}
            </strong>
          </div>
        </div>
      )}
      ...
      <div
        ...
        className="w-full text-white h-[calc(100vh-5rem-2.5rem-3.5rem)] no-scrollbar overflow-y-scroll overflow-x-clip"
      >
        <div className="flex flex-col w-full items-start py-6 gap-6">
          {items.map((item, i) => (
            ...
            >
              ...
              {i === hoverInd && itemIndex === hoverInd && hoverInd !== null ? (
                <m.div
